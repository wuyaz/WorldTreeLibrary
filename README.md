...
- 将结果绑定到当前聊天保存。
- 支持把“当前表格 / 模板 / 指令”在发送时做运行时内存注入，或注册为全局宏供用户手动插入。
...
└─ js/
   ...
   └─ ui/
      ├─ bindings.js
      ├─ blockEditor.js
      ├─ blockModalController.js
      ├─ blocks.js
      ├─ history.js
      ├─ manualWorldBook.js
      ├─ manualWorldBookState.js
      ├─ modal.js
      ├─ presetFiles.js
      ├─ presets.js
      ├─ previewTable.js
...
- [`js/ui/bindings.js`](js/ui/bindings.js) 逐步收敛为装配层，主要负责把 [`js/logic/chatState.js`](js/logic/chatState.js)、[`js/logic/injection.js`](js/logic/injection.js)、[`js/logic/statePersistence.js`](js/logic/statePersistence.js) 与各个 UI helper 串起来，并挂接剩余事件。
- [`js/ui/blocks.js`](js/ui/blocks.js) 负责提示词块 / 参考块 DOM 收集与顺序序列化。
- [`js/ui/blockEditor.js`](js/ui/blockEditor.js) 负责提示词块 / 参考块列表渲染、自定义块增删与 block 区按钮控制。
- [`js/ui/blockModalController.js`](js/ui/blockModalController.js) 负责 block 内容编辑、包装编辑与 block 预览弹窗流程。
- [`js/ui/modal.js`](js/ui/modal.js) 负责通用弹窗控制器。
- [`js/ui/tablePreview.js`](js/ui/tablePreview.js) 负责表格预览态下的 Markdown 回写、行移动、列重排、页签重排等纯处理逻辑。
- [`js/ui/previewTable.js`](js/ui/previewTable.js) 负责预览表格 tabs、通用拖拽与行拖拽绑定。
- [`js/ui/presets.js`](js/ui/presets.js) 负责文本预设与模板预设的基础 CRUD helper。
- [`js/ui/presetFiles.js`](js/ui/presetFiles.js) 负责预设文件导入 / 导出流程封装。
- [`js/ui/history.js`](js/ui/history.js) 负责历史记录存取键、历史回溯弹窗与回退/撤销回退控制器。
- [`js/ui/promptPreview.js`](js/ui/promptPreview.js) 负责 prompt 预览构建与日志区刷新装配。
- [`js/ui/presetControllers.js`](js/ui/presetControllers.js) 负责文本预设 / 模板预设选择器刷新与预设区事件绑定装配。
- [`js/ui/runtimeControllers.js`](js/ui/runtimeControllers.js) 负责批量执行、OpenAI 模型刷新、hook 管理与运行时注入控制器装配。
- [`js/ui/actionControllers.js`](js/ui/actionControllers.js) 负责自动刷新、世界书设置联动、OpenAI 预设区、主填表执行入口与主操作按钮等大块动作控制器装配。
- [`js/ui/pageControllers.js`](js/ui/pageControllers.js) 负责配置页切换、日志页签、外部模式左侧页签与发送模式联动装配。
- [`js/ui/editorControllers.js`](js/ui/editorControllers.js) 负责重置/历史控制、模板编辑区按钮、模板弹窗开关、页签拖拽与列表拖拽等成片编辑区装配，减少把大块 UI 事件继续堆回 [`js/ui/bindings.js`](js/ui/bindings.js)。
- [`js/ui/templateEditor.js`](js/ui/templateEditor.js) 负责模板编辑器弹窗开关、拖拽绑定与模板编辑器控制器封装。
- [`js/ui/manualWorldBook.js`](js/ui/manualWorldBook.js) 负责手动世界书配置界面渲染。
- [`js/ui/manualWorldBookState.js`](js/ui/manualWorldBookState.js) 负责手动世界书模板、归一化与序列化。
- [`js/api/reference.js`](js/api/reference.js) 是较早期的参考信息接口封装。
- [`js/api/memory.js`](js/api/memory.js) 是较早期的回写接口封装；当前已明确禁止写入世界书，只保留聊天绑定存储方向。
...
- [`js/logic/chatState.js`](js/logic/chatState.js) 负责聊天绑定 key、聊天元数据读写、聊天切换后的状态重载。
- [`js/logic/injection.js`](js/logic/injection.js) 负责运行时注入文本构造、发送输入框处理与注入恢复。
- [`js/logic/statePersistence.js`](js/logic/statePersistence.js) 负责 [`loadState()`](js/ui/bindings.js:998) / [`saveState()`](js/ui/bindings.js:1112) 需要的核心持久化快照与回写逻辑。
...
- `api/`：只封装外部接口和历史兼容层。
- `logic/`：只负责业务流程、状态和运行时行为，不直接持有复杂 DOM 实现。
- `table/`：只负责表格领域模型、模板、Markdown、指令解析与应用。
- `ui/`：只负责 DOM、渲染、事件装配与轻量 helper，避免把业务状态散落到界面代码中。

这意味着后续继续重构时，应优先把 [`js/ui/bindings.js`](js/ui/bindings.js) 中“可独立测试的纯逻辑”继续下沉到 [`js/logic/`](js/logic) 或 [`js/ui/`](js/ui) 下的专门模块，而不是继续堆积在一个超大文件中。
...
这些内容**不会写入世界书**。当前实现只有两种进入大模型上下文的方式：
...
1. 先根据当前配置收集需要注入的 instruction / schema / table。
...
因此 inject 模式的本质是：**只影响这一次发送，不生成本地世界书条目，也不改角色卡正文。**
...
当用户把这些宏放进预设、系统提示词或其他 ST 可展开宏的位置时，发送时会自动替换为当前内容。

### 12.3 注入顺序

运行时注入文本内部会按 position / depth / order 排序，由 [`buildManagedPromptInjectionText()`](js/ui/bindings.js:3398) 完成。

这里的 `order` 表示**同一注入层内的前后顺序**，不是“顺序靠后的内容就不注入”。只要项目启用，它们都会注入，只是排列顺序不同。

### 12.4 与世界书的关系

- 世界书仍然会被读取，作为“参考信息”来源之一。
- WorldTreeLibrary 当前**禁止把记忆表格写回世界书**。
